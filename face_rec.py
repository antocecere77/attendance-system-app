...
faceapp = FaceAnalysis(name='buffalo_sc', 
                       root = 'insight_face_model', 
                       providers=['CPUExecutionProvider'])
faceapp.prepare(ctx_id=0, det_size=(640, 640), det_thresh=0.5)

# ML Search Algorithm
# Identity Multiple Person in an image
def nl_search_algorithm(dataframe, feature_column, test_vector, name_role=['Name','Role'], thresh=0.5):
    """
...
class RealTimePred:
    ...
    def saveLogs_redis(self):
        ...
        for name, role, current_time in zip(name_list, role_list, ctime_list):
            print(name)
            print(role)
            print(current_time)
            if name != 'Unknown':
    ...
    def face_prediction(self, test_image, dataframe, feature_column, name_role=['Name','Role'], thresh=0.5):
        ...
        # step-2: use for loop and extract each embedding and pass to ml_search_alghorithm
        for res in results:
            x1,y1,x2,y2 = res['bbox'].astype(int)
            embeddings = res['embedding']
            person_name, person_role = nl_search_algorithm(dataframe, 
                                                        feature_column,
